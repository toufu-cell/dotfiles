...
| Clippings | `$VAULT/Clippings/{記事タイトル}.md` |
...
3. frontmatter の `retrieved_at` は ISO8601+TZ 形式（例: `2026-02-19T14:30:00+09:00`）で記入する
4. 信頼度・出典数に応じて `status` を設定する（下記ルール参照）
...
カテゴリ（Clippings/IT用語集/調査メモ）は関係なく、以下の表が唯一の基準:

| 出典数 | カテゴリ判定信頼度 | status |
|--------|-----------------|--------|
| 2件以上 | HIGH | `verified` |
| 2件以上 | MEDIUM/LOW | `draft` |
| 1件 | 任意 | `draft` |
| 0件 | 任意 | `draft` |

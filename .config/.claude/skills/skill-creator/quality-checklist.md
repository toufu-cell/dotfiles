...
- [ ] description に具体的なトリガー語・フレーズが含まれている
- [ ] description が「pushy」か（under-trigger 防止）— 特に自動発動型・文脈判断型
- [ ] description または本文に非適用条件（いつ使わないか）が明示されている — 誤発動抑止
- [ ] コマンド起動型: 簡潔だが、いつ使うかの1文は必須
- [ ] 発動テスト（4ケース以上）が Pass している

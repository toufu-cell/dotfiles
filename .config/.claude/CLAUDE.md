...
**tmux 優先、exec フォールバックのデュアルモード。** セッション開始時にモードを自動検出する。

- **tmux モード**: 文脈を維持した複数ラウンドの議論が可能。レビュー用途に最適。
- **exec モード**: tmux 不可時のフォールバック。ステートレス（毎回新規実行）だが、どの環境でも動作する。

#### Phase 0: モード検出（セッション開始時に1回実行）

```bash
bash ~/.claude/skills/tmux-agent/bin/codex-review-init.sh
```
- `tmux` または `exec` が出力される
- モードファイル `/tmp/codex-review-mode-${PPID}.txt` に保存される（セッション固有）
- 以降のレビュー操作はこのモードに従う

#### tmux モードの操作手順（tmux 利用可能時）
...
#### exec モードの操作手順（tmux 不可時のフォールバック）

1コマンドで完結する。文脈は保持されない（ステートレス）。

```bash
# 引数でプロンプトを渡す
bash ~/.claude/skills/tmux-agent/bin/codex-exec-review.sh "レビュー依頼のテキスト"

# 長文プロンプトは stdin で渡す（ARG_MAX 回避）
echo "長いレビュー依頼テキスト" | bash ~/.claude/skills/tmux-agent/bin/codex-exec-review.sh -
```

**exec モードの制約**:
- ステートレス: 毎回新規実行のため、再レビュー時はフルコンテキストを含める必要がある
- タイムアウト: 120秒（超過時 exit code 3）
- exit codes: 0=成功, 1=引数なし, 2=実行失敗, 3=タイムアウト

**禁止事項**:
- tmux 利用可能時に exec モードを使ってはならない。tmux が常に優先。
